import { Config, FileTypes } from "../config";
...
export const getMimeType = (name: string): string => {
   ...
   if (ext) return FileTypes[ext];
   else return "";
};

export const getMaxUploadSize = (type: string): number => {
   if (type.includes("video")) return Config.fileSizeLimit * 5;
   else if (type.includes("application")) return Config.fileSizeLimit * 5;
   else return Config.fileSizeLimit;
};
